...
const hooks = {
  ...
  'self-respect': [
    'STOP NEGOTIATING WITH YOUR OWN WORTH',
    'THIS IS YOUR REMINDER TO CHOOSE YOURSELF',
  ],
  courage: ['FEAR IS NOT A STOP SIGN', 'THE NEXT STEP DOES NOT NEED PERMISSION'],
  growth: [
    'YOUR PROGRESS MAY BE QUIETER THAN YOU THINK',
    'THE RESULT IS HIDING INSIDE THE REPETITION',
  ],
...
export function scoreShareability(quote) {
  ...
  if (/\b(quit|fear|focus|discipline|future|worth|quiet|progress|pain|hard)\b/i.test(text))
    score += 10;
...
function recipePerformance(posts, format) {
  ...
  for (const post of posts ?? []) {
    ...
    const legacyRecipe =
      GROWTH_RECIPES.find(
        (recipe) =>
          recipe.template === post.design?.template && recipe.animation === post.design?.animation,
...
export function growthDesignFor(quote, format = 'reel', cursor = 0, posts = []) {
  ...
  if (format === 'reel') {
    const mood = moodFor(quote),
      config = MOODS[mood];
    const recent = posts.filter((p) => p.format === 'reel').slice(-2);
    visual.template =
      recipe.key === 'editorial-calm'
        ? 'obsidian'
        : config.templates.find((t) => !recent.some((p) => p.design?.template === t)) ||
          config.templates[cursor % config.templates.length];
    visual.music =
      config.music.find((t) => !recent.some((p) => p.design?.music === t)) ||
      config.music[cursor % config.music.length];
  ...
  return {
    ...
    growth: {
      ...
      hook:
        format === 'reel'
          ? planReel(quote).scenes[0].text
          : pillarHooks[Math.floor(cursor / 2) % pillarHooks.length],
...
export function calculatePerformanceScore(post, metrics) {
  ...
  const raw =
    post.format === 'reel'
      ? 32 * (completion / 1.25) +
        28 * Math.min(1, shareRate / 0.03) +
        20 * Math.min(1, saveRate / 0.04) +
        12 * Math.min(1, likeRate / 0.08) +
        8 * Math.min(1, commentRate / 0.02)
      : 36 * Math.min(1, shareRate / 0.03) +
        28 * Math.min(1, saveRate / 0.04) +
        20 * Math.min(1, likeRate / 0.08) +
        16 * Math.min(1, commentRate / 0.02);

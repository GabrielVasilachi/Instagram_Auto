...
app.post('/api/worker/run', async (request, response) => {
  if (!process.env.SCHEDULER_SECRET) return response.status(503).json({ error: 'Workerul programat nu este configurat.' })
...
function normalizePostInput(body, existing = null) {
  ...
  return {
    ...
    design: { ...normalizeDesign(body.design ?? existing?.design, format), editorMedia: existing?.design?.editorMedia ?? null },
  }
}
...
app.get('/api/dashboard', async (_request, response) => {
  ...
  response.json({
    ...
    stats: {
      ...
      insightsCollected: database.posts.filter((post) => post.design?.performance?.checkedAt).length,
      averageGrowthScore: Math.round(database.posts.reduce((sum, post) => sum + (post.design?.performance?.score || 0), 0) / Math.max(1, database.posts.filter((post) => post.design?.performance?.score).length)),
    },
  })
})
...
app.post('/api/posts', async (request, response) => {
  try {
    ...
    if (values.format === 'reel') values.design.duration = planReel(values.quote, values.design).duration
...
app.patch('/api/posts/:id', async (request, response) => {
  ...
  try {
    ...
    if (updated.format === 'reel' && !existing.design?.editorMedia) updated.design.duration = planReel(updated.quote, updated.design).duration
    updated.design.reelVersion = null
    if (existing.design?.editorMedia && updated.format !== 'reel') throw new Error('Un video exportat trebuie păstrat ca Reel.')
    response.json(await savePost({ ...updated, status: 'scheduled', error: '', autoGenerated: false, mediaUrl: existing.design?.editorMedia?.url || null, retryCount: 0, nextAttemptAt: null }))
  } catch (error) {
...
app.post('/api/posts/:id/preview', async (request, response) => {
  ...
  if (post.design?.editorMedia) {
    if (!isEditorMediaUrl(post.design.editorMedia.url)) return response.status(400).json({ error: 'Fișierul Video Editor nu mai este disponibil.' })
...
app.get('/api/posts/:id/media', async (request, response) => {
  const post = await loadPost(request.params.id)
  if (!post) return response.status(404).json({ error: 'Postarea nu există.' })
  if (post.design?.editorMedia && isEditorMediaUrl(post.design.editorMedia.url)) return response.redirect(post.design.editorMedia.url)
  await mkdir(mediaDirectory, { recursive: true })
...
async function refreshOneReel(id, database) {
  if (!cloudinaryConfigured()) throw new Error('Cloudinary must be configured before refreshing scheduled media.')
  return refreshScheduledReel(id, {
    snapshot: loadRefreshSnapshot, replace: replaceRefreshedMedia,
    generate: generateMedia, upload: uploadToCloudinary, directory: mediaDirectory,
    quotes: JSON.parse(await readFile(quotesFile, 'utf8')), history: database.posts,
  })
}
...
app.post('/api/queue/refresh-reels', async (request, response) => {
  ...
  if (!ids.includes(request.body.id)) return response.status(409).json({ error: 'Postarea nu mai este eligibilă.' })
...
app.post('/api/queue/rebuild', async (_request, response) => {
  ...
  if (!acquired) {
    return response.status(409).json({ error: 'Workerul publică sau completează coada. Reîncearcă în câteva secunde.' })
  }
  ...
  try {
    ...
    await releaseWorkerLease('instagram-publisher', owner)
      .catch((error) => console.error('[queue-lease]', error instanceof Error ? error.message : error))
  }
})
...
app.post('/api/insights/refresh', async (request, response) => {
  ...
  if (!acquired) return response.status(409).json({ error: 'Workerul rulează deja. Reîncearcă în câteva secunde.' })
  ...
  try {
    ...
    response.status(400).json({ error: error instanceof Error ? error.message : 'Insights nu au putut fi actualizate.' })
  } finally {
    await releaseWorkerLease('instagram-publisher', owner)
      .catch((error) => console.error('[insights-lease]', error instanceof Error ? error.message : error))
  }
})
...
async function publishPost(postId) {
  ...
  try {
    ...
    let publicUrl
    if (post.design?.editorMedia) {
      if (!isEditorMediaUrl(post.design.editorMedia.url)) throw new Error('Fișierul Video Editor nu este valid. Exportă din nou proiectul.')
...
async function collectNextMediaInsight() {
  ...
  const candidate = database.posts
    .filter((post) => post.status === 'published'
      ...
      && (!post.design?.performance?.checkedAt || now - new Date(post.design.performance.checkedAt).getTime() >= 6 * 60 * 60_000))
    .sort((left, right) => {
  ...
  return { id: candidate.id, format: candidate.format, recipe: candidate.design?.growth?.recipe || null, ...metrics, ...rating, checkedAt }
}
...
if (process.env.REFRESH_REELS === 'true') {
  const database = await loadDatabase()
  const candidates = database.posts.filter(post => eligibleForRefresh(post) && post.design?.reelVersion !== REEL_VERSION)
  ...
  for (const post of candidates) {
    ...
    } catch (error) { failed++; console.error(JSON.stringify({ id: post.id, status: 'failed', error: error.message })) }
  }
...
export { collectNextMediaInsight, fillQueue, publishDuePosts, publishDueStories, publishPost, publishStoryPromotion, runRemoteWorker }
export default app

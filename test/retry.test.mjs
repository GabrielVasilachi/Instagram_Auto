...
import { isRetryablePublishError, isTransientRemoteError, retryDelayMs, withRemoteRetries } from '../server/retry.mjs'

test('Supabase clock-skew errors are retried until the operation succeeds', async () => {
  let calls = 0
  const waits = []
  const result = await withRemoteRetries(async () => {
    calls += 1
...
test('non-transient publishing errors fail immediately', async () => {
  let calls = 0
  await assert.rejects(withRemoteRetries(async () => {
    calls += 1
    throw new Error('Instagram permission denied')
  }, { wait: async () => {} }), /permission denied/)
...
test('publishing retries common temporary Instagram and Cloudinary errors', () => {
  ...
  assert.deepEqual([1, 2, 3, 4, 8].map(retryDelayMs), [60_000, 300_000, 900_000, 3_600_000, 3_600_000])
})

...
test('worker preflight rejects legacy Supabase JWT keys', () => {
  assert.throws(
    () => validateWorkerEnvironment({ ...validEnvironment, SUPABASE_SECRET_KEY: 'eyJlegacy' }),
    /cheie Supabase veche/,
  );
});

test('remote scheduler requires its own secret', () => {
  assert.throws(
    () => validateWorkerEnvironment(validEnvironment, { scheduler: true }),
    /SCHEDULER_SECRET/,
  );
  assert.equal(
    validateWorkerEnvironment(
      { ...validEnvironment, SCHEDULER_SECRET: 'scheduler-secret' },
      { scheduler: true },
    ).schedulerProtected,
    true,
  );

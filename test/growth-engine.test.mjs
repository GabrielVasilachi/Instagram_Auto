...
import { calculatePerformanceScore, detectPillar, growthDesignFor, scoreShareability } from '../server/growth-engine.mjs'
...
test('performance score rewards retention, shares and saves', () => {
  ...
  const weak = calculatePerformanceScore(post, { reach: 500, averageWatchTimeMs: 1200, shares: 0, saved: 0, likes: 4, comments: 0 })
  const strong = calculatePerformanceScore(post, { reach: 500, averageWatchTimeMs: 6200, shares: 18, saved: 22, likes: 45, comments: 8 })
...
test('legacy posts teach the engine through their matching visual template', () => {
  const posts = [
    { format: 'reel', design: { template: 'midnight', animation: 'drift', performance: { reach: 500, score: 92, confidence: 100 } } },
    { format: 'reel', design: { template: 'paper', animation: 'pan', performance: { reach: 500, score: 72, confidence: 100 } } },
  ]

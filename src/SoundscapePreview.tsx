...
function Sample({ music, volume }: { music: string; volume: number }) {
  ...
  if (music === 'silent')
    return <p className="settings-note">Fără muzică — opțiunea „silent” nu conține audio.</p>;
  return (
    <div className="soundscape-preview">
      ...
      <audio
        ...
        controls
        preload="none"
        aria-label={`Ascultă soundscape ${music}`}
      ...
      {error && (
        <div role="alert">
          <small>Mostra nu a putut fi încărcată.</small>{' '}
          <button
            type="button"
            onClick={() => {
              setError(false);
